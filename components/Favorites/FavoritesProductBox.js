...
export default function FavoritesProductBox(props, {navigation}) {
  const deleteLikebyid = async state => {
    try {
      const res = await axios
        .post('http://192.168.43.121/api/Like/deleteLike', state)
        .then(res => res.data)
        .then(data => {
          console.log(data);
          Alert.alert('Delete From your favorite');
        });
    } catch (e) {
      console.log(e);
    }
  };
  const Delete = namefood => {
    ClickDelete(namefood);
  };
  const ClickDelete = namefood => {
    const state = {
      name: namefood,
      id_user: props.userid,
    };
    deleteLikebyid(state);
  };
